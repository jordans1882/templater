...
# version of the templater package... "like a boss".
build: attrib
	R CMD build .
...
#
# 	  > make TGZ=glandmaker_x.y.z.tar.gz check
check:
...
veryclean: clean
	rm -f src/templater.so
	rm -f templater_*.tar.gz
	rm -f templater.pdf
	rm -rf templater.Rcheck
